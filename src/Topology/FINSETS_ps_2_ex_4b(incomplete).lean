...
-- We need to use finsets to do this more easily. We can use the functions that turn sets into finsets and back in order to agree with the definition of a topology, which uses sets.
@[derive decidable_eq] inductive zeroonetwo : Type
| zero : zeroonetwo 
| one : zeroonetwo
| two : zeroonetwo
...
instance : fintype zeroonetwo :=
⟨{zero, one, two}, λ x, by cases x; simp⟩

local attribute [instance] fintype.set_fintype

example : ({zero, one} : finset zeroonetwo) ∩ {zero} = {zero} := dec_trivial

def collection_one : finset zeroonetwo  → Prop := λ s, s = ∅ ∨ s = {one, two} ∨ s = {zero, one, two}
def collection_two : finset zeroonetwo → Prop := λ s, s = ∅ ∨ s = {zero, one} ∨ s = {zero, one, two} 
