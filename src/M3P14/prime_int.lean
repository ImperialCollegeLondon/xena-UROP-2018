...
Prime numbers.
-/
import data.nat.sqrt data.nat.gcd data.int.basic data.list.perm
open bool subtype
...
def prime (p : ℤ) := p ≥ 2 ∧ ∀ m ∣ p, m = 1 ∨ m = p

theorem prime.ge_two {p : ℤ} : prime p → p ≥ 2 := and.left

theorem prime.gt_one {p : ℤ } : prime p → p > 1 := prime.ge_two

theorem prime_def_lt {p : ℤ } : prime p ↔ p ≥ 2 ∧ ∀ m < p, m ∣ p → m = 1 :=
...
#check 
theorem prime_def_lt' {p : \int } : prime p ↔ p ≥ 2 ∧ ∀ m, 2 ≤ m → m < p → ¬ m ∣ p :=
...
theorem prime_def_le_sqrt {p : \int } : prime p ↔ p ≥ 2 ∧
  ∀ m, 2 ≤ m → m ≤ sqrt p → ¬ m ∣ p :=
...
def decidable_prime_1 (p : \int ) : decidable (prime p) :=
decidable_of_iff' _ prime_def_lt'
local attribute [instance] decidable_prime_1

theorem prime.pos {p : \int } (pp : prime p) : p > 0 :=
...
section min_fac
  ...
  private def min_fac_prop (n k : \int
 ) :=
    k ≥ 2 ∧ k ∣ n ∧ ∀ m ≥ 2, m ∣ n → k ≤ m

  theorem min_fac_aux_has_prop {n : \int
 } (n2 : n ≥ 2) (nd2 : ¬ 2 ∣ n) :
  ...
  theorem min_fac_has_prop {n : \int
 } (n1 : n ≠ 1) :
  ...
  theorem min_fac_pos (n : \int
 ) : min_fac n > 0 :=
  by by_cases n1 : n = 1;
     [exact n1.symm ▸ dec_trivial, exact (min_fac_prime n1).pos]

  theorem min_fac_le {n : \int
 } (H : n > 0) : min_fac n ≤ n :=
  le_of_dvd H (min_fac_dvd n)

  theorem prime_def_min_fac {p : \int
 } : prime p ↔ p ≥ 2 ∧ min_fac p = p :=
  ⟨λ pp, ⟨pp.ge_two,
  ...
  instance decidable_prime (p : \int
 ) : decidable (prime p) :=
  decidable_of_iff' _ prime_def_min_fac

  theorem not_prime_iff_min_fac_lt {n : \int
 } (n2 : n ≥ 2) : ¬ prime n ↔ min_fac n < n :=
...
theorem prime.coprime_iff_not_dvd {p n : \int } (pp : prime p) : coprime p n ↔ ¬ p ∣ n :=
⟨λ co d, pp.not_dvd_one $ co.dvd_of_dvd_mul_left (by simp [d]),
 λ nd, coprime_of_dvd $ λ m m2 mp, ((dvd_prime_ge_two pp m2).1 mp).symm ▸ nd⟩

theorem prime.dvd_iff_not_coprime {p n : \int } (pp : prime p) : p ∣ n ↔ ¬ coprime p n :=
iff_not_comm.2 pp.coprime_iff_not_dvd

theorem prime.dvd_mul {p m n : \int } (pp : prime p) : p ∣ m * n ↔ p ∣ m ∨ p ∣ n :=
...
theorem prime.not_dvd_mul {p m n : \int } (pp : prime p)
  (Hm : ¬ p ∣ m) (Hn : ¬ p ∣ n) : ¬ p ∣ m * n :=
mt pp.dvd_mul.1 $ by simp [Hm, Hn]

theorem prime.dvd_of_dvd_pow {p m n : \int } (pp : prime p) (h : p ∣ m^n) : p ∣ m :=
by induction n with n IH;
   [exact pp.not_dvd_one.elim h,
    exact (pp.dvd_mul.1 h).elim IH id]

theorem prime.coprime_pow_of_not_dvd {p m a : \int } (pp : prime p) (h : ¬ p ∣ a) : coprime a (p^m) :=
(pp.coprime_iff_not_dvd.2 h).symm.pow_right _

theorem coprime_primes {p q : \int } (pp : prime p) (pq : prime q) : coprime p q ↔ p ≠ q :=
pp.coprime_iff_not_dvd.trans $ not_congr $ dvd_prime_ge_two pq pp.ge_two

theorem coprime_pow_primes {p q : \int } (n m : \int ) (pp : prime p) (pq : prime q) (h : p ≠ q) :
  coprime (p^n) (q^m) :=
((coprime_primes pp pq).2 h).pow _ _

theorem coprime_or_dvd_of_prime {p} (pp : prime p) (i : \int ) : coprime p i ∨ p ∣ i :=
by rw [pp.dvd_iff_not_coprime]; apply em

theorem dvd_prime_pow {p : \int } (pp : prime p) {m i : \int } : i ∣ (p^m) ↔ ∃ k ≤ m, i = p^k :=
begin
...
lemma mem_list_primes_of_dvd_prod {p : \int } (hp : prime p) :
  ∀ {l : list \int
 }, (∀ p ∈ l, prime p) → p ∣ prod l → p ∈ l
...
lemma mem_factors_of_dvd {n p : \int } (hn : 0 < n) (hp : prime p) (h : p ∣ n) : p ∈ factors n :=
mem_list_primes_of_dvd_prod hp (@mem_factors n) ((prod_factors hn).symm ▸ h)

lemma perm_of_prod_eq_prod : ∀ {l₁ l₂ : list \int }, prod l₁ = prod l₂ →
  (∀ p ∈ l₁, prime p) → (∀ p ∈ l₂, prime p) → l₁ ~ l₂
| []        []        _  _  _  := perm.nil
| []        (a :: l)  h₁ h₂ h₃ :=
  have ha : a ∣ 1 := @prod_nil \int
  _ ▸ h₁.symm ▸ (@prod_cons _ _ l a).symm ▸ dvd_mul_right _ _,
  absurd ha (prime.not_dvd_one (h₃ a (mem_cons_self _ _)))
| (a :: l)  []        h₁ h₂ h₃ :=
  have ha : a ∣ 1 := @prod_nil \int
  _ ▸ h₁ ▸ (@prod_cons _ _ l a).symm ▸ dvd_mul_right _ _,
  absurd ha (prime.not_dvd_one (h₂ a (mem_cons_self _ _)))
| (a :: l₁) (b :: l₂) h hl₁ hl₂ :=
...
lemma factors_unique {n : \int } {l : list \int } (h₁ : prod l = n) (h₂ : ∀ p ∈ l, prime p) : l ~ factors n :=
