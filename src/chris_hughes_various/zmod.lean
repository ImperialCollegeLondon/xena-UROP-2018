...
lemma int.coe_nat_ne_zero_iff_pos {n : ℕ} : (n : ℤ) ≠ 0 ↔ 0 < n :=
⟨λ h, nat.pos_of_ne_zero $ int.coe_nat_ne_zero.1 h,
  λ h, ne.symm $ ne_of_lt $ int.coe_nat_lt.2 h⟩

lemma int.coe_nat_nonneg (n : ℕ) : (0 : ℤ) ≤ n :=
int.coe_nat_le.2 $ nat.zero_le _
