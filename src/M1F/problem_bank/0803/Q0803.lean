import algebra.group_power data.nat.basic
open nat
...
theorem Q0803ia (e : ℕ) : 10^e % 9 = 1 := sorry
-- sum of digits
lemma digit_sum_aux (n : ℕ) : succ n / 10 < succ n := sorry 
...
theorem Q0803ib (N a : ℕ) (hp: a = s N) : 9 ∣ N - a := sorry 
theorem Q0803ic (N a : ℕ) (hp : a = s N) : 9 ∣ N ↔ 9 ∣ a := sorry
theorem Q0803id : 9 ∣ 123456789 := sorry 

--(ii) Deduce from (i) that N is a multiple of 3 if and only if s is.
theorem Q0803ii (N a : ℕ) (hp : a = s N) : 3 ∣ N ↔ 3 ∣ a := sorry 
...
theorem Q0803iii (r e : ℕ) (hp : e ≥ r) : 10^e % 2*r = 0 := sorry
...
theorem Q0803iv (e : ℕ) : 10^e % 11 = (-1 : ℤ)^e := sorry
--theorem Q